...
export async function createReservation(
  ...
  const { data, error } = await supabase
    .from("reservation_requests")
    .insert({ ...input, status: "pending_payment", payment_session_id: null })
    .select("*")
    .single();
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const checkAdmin = useCallback(async (nextUser: User | null) => {
    ...
    if (
      nextUser.app_metadata?.role === "admin" ||
      nextUser.user_metadata?.role === "admin"
    ) {
      setIsAdmin(true);
      return;
    }
  ...
  const value = useMemo<AuthContextValue>(
    ...
    [session, user, role, isAdmin, loading, signIn, signUp, signOut],
  );
