...
const PLACEHOLDER_ADMIN_EMAIL = "replace-with-real-admin-email@example.com";
const DEFAULT_ADMIN_EMAIL = "admin@headspa.cz";
const DEFAULT_ADMIN_PASSWORD = "HeadSpa123!";

const ADMIN_EMAILS = (import.meta.env.VITE_ADMIN_EMAILS ?? "")
  .split(",")
  .map((v) => v.trim().toLowerCase())
  .filter((value) => value && value !== PLACEHOLDER_ADMIN_EMAIL);
...
function ensureDemoAdminUser(): LocalAuthUser[] {
  if (typeof window === "undefined") return [];

  const existingUsers = safeReadUsers();
  const normalizedEmail = normalizeEmail(DEFAULT_ADMIN_EMAIL);
  const hasDemoAdmin = existingUsers.some(
    (entry) => normalizeEmail(entry.email) === normalizedEmail,
  );

  if (hasDemoAdmin) return existingUsers;

  const demoAdmin: LocalAuthUser = {
    id: "demo-admin",
  ...
  safeWriteUsers([...existingUsers, demoAdmin]);
  return safeReadUsers();
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signIn = useCallback(async (email: string, password: string) => {
    const users = ensureDemoAdminUser();
