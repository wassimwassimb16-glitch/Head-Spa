...
var ClientEventBus = class {
	...
	constructor({ port = getDefaultPort(4206), host = getDefaultHost("localhost"), protocol = getDefaultProtocol("http"), debug = false, connectToServerBus = false } = {}) {
		...
		this.#broadcastChannel.onmessage = (e) => {
			this.emitToClients(require_json.parseWithBigInt(e.data), true);
		};
	...
	emitToClients(event, fromBroadcastChannel = false) {
		...
		if (!fromBroadcastChannel) this.#broadcastChannel?.postMessage(require_json.stringifyWithBigInt(event));
	...
	emitToServer(event) {
		const json = require_json.stringifyWithBigInt(event);
		if (this.#socket) {
	...
	handleEventReceived(data) {
		try {
			const event = require_json.parseWithBigInt(data);
			this.emitToClients(event);
		} catch {}
	}
};
