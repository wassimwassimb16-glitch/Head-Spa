import { createRequiredError, normalizeKey } from "../_chunks/utils.mjs";
...
const driver = (opts = {}) => {
	...
	if (!storage) throw createRequiredError(DRIVER_NAME, "localStorage");
	const base = opts.base ? normalizeKey(opts.base) : "";
	const r = (key) => (base ? `${base}:` : "") + key;
	let _storageListener;
	const _unwatch = () => {
		if (_storageListener) opts.window?.removeEventListener("storage", _storageListener);
	...
	return {
		...
		clear(prefix) {
			...
			if (_base) {
				for (const key of Object.keys(storage)) if (key.startsWith(`${_base}:`)) storage?.removeItem(key);
			} else storage.clear();
		},
		dispose() {
			if (opts.window && _storageListener) opts.window.removeEventListener("storage", _storageListener);
		},
		watch(callback) {
			if (!opts.window) return _unwatch;
			_storageListener = (ev) => {
				if (ev.key) callback(ev.newValue ? "update" : "remove", ev.key);
			};
